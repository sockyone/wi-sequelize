...
const sqlTransform = require('./sqlTransform');
  ...
  run(sql, parameters) {
    ...
    return new Utils.Promise((resolve, reject) => {
      const handler = (err, results) => {
        ...
        if (err) {
          err.sql = sql;
          if (this.connection.config.database!= 'wi_backend') {
            console.log('Handler work:', err.message);
          }
      ...
      if (parameters) {
        ...
        if (this.connection.config.database != 'wi_backend' && sql.indexOf('INSERT INTO') == 0) {
          this.sql = sqlTransform(this.sql);
        }
        //console.log(this.sql);
        this.connection.query({ sql:
          this.sql }, handler).setMaxListeners(100);
      }
    })
    ...
    // Return formatted results...
      .then((results) => {
        messageTransfer.push(this.connection.config.database, this.sql);
